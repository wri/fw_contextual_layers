...
class LossLayerProtocol {
  ...
  static filterImgData(data, { w, h, z, startYear, endYear }) {
    ...
    for (let i = 0; i < w; ++i) {
      for (let j = 0; j < h; ++j) {
        ...
        const intensity = data[pixelPos];
        const yearLoss = 2000 + data[pixelPos + 2];

        if (yearLoss >= startYear && yearLoss < endYear) {
          data[pixelPos] = 220;
          data[pixelPos + 1] = 72 - z + 102 - (3 * myscale(intensity)) / z;
          data[pixelPos + 2] = 33 - z + 153 - intensity / z;
          data[pixelPos + 3] = z < 13 ? myscale(intensity) : intensity;
        } else {
          data[pixelPos + 3] = 0;
        }
      }
  ...
  async getImageTile({ z, x, y, startYear, endYear }, format = "image/png") {
    ...
    const imageString = await canvasTile.toDataURL(format);
    return Buffer.from(imageString.split(",")[1], "base64");
  }
}
